...
function addGamesToPage(games) {
    ...
    for (var i = 0; i < games.length; i++) {
        ...
        let gcardDiv = document.createElement("div");

        // add the class game-card to the list
        gcardDiv.classList.add("game-card");
        ...
        gamesContainer.appendChild(gcardDiv);
    }
...
let totalRaisedF = totalRaised.toLocaleString('en-US');
let unfundedStr = `A total of $${totalRaisedF} has been raised for ${GAMES_JSON.length} ${GAMES_JSON.length > 1 ? "games" : "game"}.
                    Currently, ${countUnfunded} ${countUnfunded > 1 ? "games" : "game"} remain unfunded. We need your help to fund 
                    these amazing games!`;

// create a new DOM element containing the template string and append it to the description container
let pDiv = document.createElement("div");
pDiv.innerHTML = `<p>${unfundedStr}</p>`;
